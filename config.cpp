...
<head>
  ...
  <script>
    ...
    function led(r, g, b) {
      ...
      xhttp.open("GET", `/led?duty=${r},${g},${b}`, true) ;
      xhttp.send() ;
    }

    function tail(value) {
      var xhttp = new XMLHttpRequest() ;
      xhttp.open("GET", `/tail?duty=${value}`, true) ;
      xhttp.send() ;
    }

    function head(ver, hor) {
      var xhttp = new XMLHttpRequest() ;
      xhttp.open("GET", `/head?duty=${ver},${hor}`, true) ;
      xhttp.send() ;
    }
  </script>
  <style>
...
<body class="noselect" onload="init();">
  <header>
    <div class="container noselect">
      ...
      <div>
        <nobr>LED Light : </nobr>
        <input type="range" id="slider" min="0" max="255" value="0" step="10" onchange="led(this.value, this.value, this.value)">
        <nobr>Servo : </nobr>
        <input type="range" id="slider" min="0" max="100" value="100" step="10" onchange="tail(this.value)">
      </div>
    </div>
  </header>
  <div>
